...
(defconst +tt-test--state
  '((meta (title . "sum validation"))
    ...
    (decisionStatuses (D-p1-7c1e0a4a-1 (status . "failed") (reason . "M veto"))
                      (D-p1-7c1e0a4a-disc-A-2 (status . "passed"))
...
(ert-deftest tradeoffs-trace-decision-render ()
  ...
  (with-temp-buffer
    ...
    (let ((text (buffer-string)))
      ...
      (should (string-match-p "\\* ACCEPTED  Errors are thrown, not returned" text))
    ...
    (+tt--render-status-from
     `((meta (title . "sum validation"))
       ...
       (view (elapsed . "1m02s") (round . 1)
             ...
             (liveDecisions . 4) (failedDecisions . 0) (openFindings . 1) (boundaryFilesChanged . 2)))
     "/tmp/tt-ert/abcd1234")
    (let ((text (buffer-string)))
